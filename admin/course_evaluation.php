          ...
          <div class="row">
            <div class="col-md-12 grid-margin">
              <div class="d-flex justify-content-between align-items-center">
                <div>
                  <h4 class="font-weight-bold mb-0">Course Evaluation Evaluation</h4>
                </div>
...
<?php
$result=mysqli_query($conn,"SELECT distinct(lecturer),course,score FROM course_feedback group by lecturer order by sum(score) desc ");
...
<td>
  <?php
  $course= $rows['course'];
  $points5=mysqli_query($conn,"SELECT distinct(adm_no) FROM course_feedback WHERE lecturer='$namo' AND course='$course'");
...
$resultscore=mysqli_query($conn,"SELECT SUM(score) AS totalscore FROM course_feedback WHERE lecturer='$namo' and course='$course'  ");
...
$percentage=mysqli_query($conn,"SELECT * FROM course_feedback ");
...
<td><a href="course_detailed_analysis.php?unique_no=<?php echo $namo ?>&&course=<?php echo  $rows['course']?>&&name=<?php echo $resultsm1['name'] ?>">Detailed Analysis </td>
